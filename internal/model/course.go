...
func ParseCourseList(data []byte) (*CourseList, error) {
	...
	for _, item := range result.TmpList {
		...
		// Bug Fix: 补充解析 zrs（总人数/Capacity）
		// 原代码遗漏了此字段，导致 IsFull() 中 Capacity 始终为 0，
		// 所有课程都被判断为"未满"，无法正确跳过满员课程。
		if v, ok := item["zrs"].(float64); ok {
			course.Capacity = int(v)
		}
		if v, ok := item["jsm"].(string); ok {
			course.Teacher = v
		}
		if v, ok := item["jsmc"].(string); ok {
			course.Room = v
		}
		if v, ok := item["sksj"].(string); ok {
			course.WeekTime = v
		}
