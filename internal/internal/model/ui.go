...
// courseTypeDisplayToCode 将 UI 显示名转换为内部代码
//
// Bug Fix: 原代码直接把 RadioGroup 的显示文字（"普通网课"/"体育课"/"普通课"）
// 赋值给 cfg.CourseType，但 Match() 函数判断的是内部代码 "online"/"pe"/"normal"，
// 导致课程类型筛选完全失效（所有课程都被认为匹配）。
func courseTypeDisplayToCode(display string) string {
	switch display {
	case "普通网课":
		return "online"
	case "体育课":
...
// courseTypeCodeToDisplay 将内部代码转换为 UI 显示名（用于 SetConfig）
func courseTypeCodeToDisplay(code string) string {
	switch code {
	case "online":
		return "普通网课"
	case "pe":
...
func (ui *UIComponents) GetConfig() *Config {
	...
	// Bug Fix: 将 UI 显示名（"普通网课"/"体育课"/"普通课"）转换为内部代码（"online"/"pe"/"normal"）
	// 原代码直接赋值显示文字，导致 Match() 中课程类型筛选永远不匹配
	cfg.CourseType = courseTypeDisplayToCode(ui.CourseTypeRadio.Selected)
...
func (ui *UIComponents) SetConfig(cfg *Config) {
	...
	// Bug Fix: 将内部代码转换为 UI 显示名再设置
	ui.CourseTypeRadio.SetSelected(courseTypeCodeToDisplay(cfg.CourseType))
