...
func (a *App) onStartClicked() {
	...
	// 2. 明文 HTTP 内网节点：学号与 Session Cookie 明文传输；
	//    密码**并非**安全 —— 它虽经 RSA 加密，但加密用的公钥同样经由
	//    这条明文链路下发（fetchPublicKeyFromAPI → /xtgl/login_getPublicKey.html），
	//    同网段攻击者可以把公钥替换成自己的，从而还原明文密码。
	//    因此"密码有 RSA 保护"在明文节点上并不成立，警告文案必须如实告知。
	//    （代码侧已拒绝过短的弱公钥，但无法识别一把数学上合法的替换公钥。）
	//
