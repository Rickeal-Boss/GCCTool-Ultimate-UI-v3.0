...
新增 DelayUltra 极速模式
抢课阶段自动切换到极速模式
并发启动所有 Worker（0ms 启动延迟）
![img.png](newexamplespreview.png)
**2. 精准风控检测**
登录阶段：完整风控检测（包括验证码）
正常速度（200~600ms 延迟）
抢课阶段：
...
本轮针对「抢不到课」与「误报/误导」两类问题做了系统性修复。

**功能链路（直接影响能否抢到课）**

- **课程类型中英文不匹配**：UI 展示的中文标签被直接当成业务值，与 `online/pe/normal` 判定不一致 → 类型过滤完全失效（选体育课也会去抢网课），且查询类别恒为网课。现改为单一映射源（`internal/model/course_type.go`），中文标签只出现在展示层
- **选课提交参数补全**：改为回填「选课首页 / Display 页」原样下发的初始化参数（`xkkz_id` 等），不再只提交 5 个字段
- **课程详情接口兼容数组 / 对象两种响应形态**；详情获取失败时降级用短 `jxb_id` 重试，不再直接跳过整门课
- **选课结果 `flag` 支持数字与字符串两种类型**，并补上 `flag = -1`（服务端要求重试）分支
- **课程字段解析容错**：`xf` / `zrs` / `yxzrs` 等字段为字符串型数值时不再被静默归零

**稳定性与安全**

- `doPost` 补齐 HTTP 状态码校验；响应解压支持 `deflate`（此前只处理 `gzip`）
- 登录状态校验不再把「被重定向回登录页」误判为登录成功（移除 `xtgl` 等必然命中的误报标志）
- 抢课模式保留「账号封禁 / Session 失效 / 429 限流」三类致命检测（此前只检测封禁）
- 空结果语义拆分：区分「服务端 0 门课 / 筛选条件全不命中 / 候选全部满员」，分别使用不同的日志级别与重试节奏
- 修复 `WaitGroup` 被并发重置导致的数据竞争
- 日志脱敏函数接入主链路（此前已定义但从未被调用）
- 移除会破坏自动重新登录的「密码清零」代码（该 `cfg` 与 `robber.config` 是同一指针）

**工程**

- 删除会破坏 `go build ./...` 的 `examples/` 残留文件与无任何引用的 `data.json`
- `build.sh` 输出文件名按平台决定扩展名（此前硬编码 `.exe`）
- 新增 `.gitignore`，避免本地配置与构建产物被提交
